...
void InsertStudentIntoSchoolPreferenceList(std::istream &istr, std::list<School> &schools) {
    ...
    std::list<School>::iterator i;
    for (i=schools.begin(); i!=schools.end(); i++){
        if (i->GetName()==school_name){
            i->InsertStudent (new_student_name, insert_before_student_name);
        }
    }
...
void RemoveSchoolFromStudentPreferenceList(std::istream &istr, std::list<Student> &students) {
    ...
    for (i=students.begin(); i!=students.end(); i++){
        if (i->GetName()==student_name){
            i->RemoveSchool(school_name);
        }
    }
...
void PrintSchoolPreferenceList(std::istream &istr, const std::list<School> &schools, std::ostream &ostr) {
    ...
    istr >> school_name;
    std::list<School>::const_iterator i;
    std::list<std::string> s;
    for(i=schools.begin(); i!=schools.end();i++){
        if (i->GetName()==school_name){
            s=i->get_student_list();
            break;
        }
    }
    ostr << "the preferenceList for "<< school_name <<" are: " <<std::endl;
...
void PrintStudentPreferenceList(std::istream &istr, const std::list<Student> &students, std::ostream &ostr) {
    ...
    istr >> student_name;
    std::list<Student>::const_iterator i;
    std::list<std::string> s;
    for(i=students.begin(); i!=students.end(); i++){
        if (i->GetName()==student_name){
            s=i->get_school_list();
    ...
    ostr << "the preferenceList for "<< student_name << "are: "<<std::endl;
    std::list<std::string>::iterator ii;
    for (ii=s.begin(); ii!=s.end();ii++){
        ostr << *ii << std::endl;
    }
